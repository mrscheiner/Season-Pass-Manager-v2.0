...
import { parseScheduleFromCSV, parseScheduleFromExcel, validateSchedule } from '@/lib/scheduleImporter';
import * as DocumentPicker from 'expo-document-picker';

const BACKUP_VERSION = '1.0';
const AUTO_RESTORE_FLAG = 'AUTO_RESTORE_COMPLETED';

// Cloud sync (last-backup-wins)
const CLOUD_SYNC_ENABLED_KEY = 'cloud_sync_enabled_v1';
const CLOUD_SYNC_SECRET_KEY = 'cloud_sync_secret_v1';
const CLOUD_SYNC_LAST_SERVER_UPDATED_AT_KEY = 'cloud_sync_last_server_updated_at_v1';
const CLOUD_SYNC_AUTO_UPLOAD_ENABLED_KEY = 'cloud_sync_auto_upload_enabled_v1';
const CLOUD_SYNC_AUTO_DOWNLOAD_ENABLED_KEY = 'cloud_sync_auto_download_enabled_v1';
const CLOUD_SYNC_LAST_LOCAL_MUTATION_AT_KEY = 'cloud_sync_last_local_mutation_at_v1';
const CLOUD_SYNC_LAST_SYNCED_LOCAL_MUTATION_AT_KEY = 'cloud_sync_last_synced_local_mutation_at_v1';
...
async function fetchScheduleViaESPN(pass: {
  ...
  console.log('[ScheduleFetch] API base:', baseUrl || 'NOT SET', `(source: ${resolvedApi.source})`);
  
  if (!baseUrl) {
    console.error('[ScheduleFetch] ❌ API base is undefined!');
...
async function fetchScheduleViaTicketmaster(pass: {
  ...
  const baseUrl = resolveApiBaseUrl().baseUrl;
  if (!baseUrl) {
    return { games: [], error: 'NETWORK' };
  }
...
export interface BackupData {
  ...
  // Standalone misc events (concerts/one-off tickets) that should sync and restore with the app.
  miscEvents?: Event[];
  // Tracks when the user last generated a restoreable backup artifact.
  lastUserBackupAtISO?: string | null;
...
function buildAppSnapshotMarkdown(): string {
  // This is a best-effort, code-derived description of the current UI and behaviors.
  // It is intentionally text-only (no screenshots) so it can be emailed/copied easily.
  return [
    '# Season Pass Manager — App Snapshot (Generated)',
    '',
    'This document is generated by the app at backup time. It is intended as a “rebuild spec” so another environment (e.g., Rork) can reproduce the same behavior.',
    '',
    '## What this CAN and CANNOT clone',
    '- ✅ Data clone: season passes, seats, sales, schedule, events, theme snapshot, optional embedded logos.',
    '- ❌ Code clone: screens/components/styling/dependencies are NOT included in data backups. Use Git for a true app-code snapshot.',
    '',
    '## App structure / navigation',
    '- Main tabs: Dashboard, Schedule, Analytics, Events, Settings.',
    '- Other screens: Setup (create season pass), Restore (from code), Edit Season Pass (edit seats & cost).',
    '',
    '## Core data model (high-level)',
    '- SeasonPass',
    '  - Seat entries (“seatPairs”): section, row, seats string, seasonCost (price paid).',
    '  - Games: opponent, date/time, type (Preseason/Regular/Playoff), and optional opponentLogo.',
    '  - Sales: keyed by gameId then seatPairId, containing price, paymentStatus, soldDate, etc.',
    '- Misc Events (standalone, NOT part of any SeasonPass)',
    '  - name/date/paid/sold/status (used only in the Events tab; never affects Analytics).',
    '',
    '## Colors / theme',
    '- Global palette: `constants/appColors.ts`.',
    '- Main tab headers use the active pass primary color when present.',
    '',
    '## Setup (create a season pass)',
    '- Step flow: League → Team → Season → Seats → Confirm.',
    '- Seat entry mode:',
    '  - Paired: requires exactly 2 seats (examples: `24-25`, `24,25`).',
    '  - Individual: supports 1+ seats; auto-switches to Individual when >2 seats entered.',
    '- Clickables:',
    '  - League card, Team card: selects and advances.',
    '  - Back: navigates step backward.',
    '  - Add seat entry: appends the draft entry; Remove: deletes it from the draft.',
    '  - Confirm: creates the pass and routes to tabs.',
    '',
    '## Dashboard tab',
    '- Header: season pass selector + team name + season label.',
    '- Stat cards:',
    '  - Total Revenue: sum of all sale prices.',
    '  - Seats Sold: tap opens All Sales modal.',
    '  - Avg Price: average price per seat.',
    '  - Pending: number of pending-payment sale records.',
    '- Recent Sales list: last 5 sales with opponent logo + seat info + sold date + price + status badge.',
    '- Clickables:',
    '  - SeasonPassSelector: switches active pass.',
    '  - Seats Sold card: opens All Sales modal.',
    '  - “View All (N)” in Recent Sales: opens All Sales modal.',
    '  - All Sales modal: close button (X).',
    '',
    '## Schedule tab',
    '- Search:',
    '  - Matches opponent/date/time/game number.',
    '  - Also matches sale fields: section/row/seats/price.',
    '- Filters: All Games / Preseason / Regular / Playoff.',
    '- Game list:',
    '  - Gradient cards (Panthers palette by default) with past games visually muted.',
    '  - Shows date tile, opponent logo, game #, opponent name, time, and “seats available”.',
    '  - Shows Paid/Pending badge indicating whether ALL seat entries for that game are paid.',
    '- Clickables:',
    '  - Filter pill: changes game list filter.',
    '  - Game card: opens Game Sales modal (pageSheet).',
    '  - Game Sales modal:',
    '    - X: close without forcing saves.',
    '    - Top-right Check: save-all-and-close.',
    '    - For each seat entry:',
    '      - Paid/Pending toggle changes payment status.',
    '      - Price input edits sale price.',
    '      - Check button saves that one sale entry immediately.',
    '    - Clearing the price for an existing sale removes that sale record on save-all.',
    '',
    '## Analytics tab',
    '- Season Overview cards:',
    '  - Total Revenue, Seats Sold, Sold Rate.',
    '- Monthly Revenue chart:',
    '  - Month bucket is based on each sale’s soldDate (not the game date).',
    '  - Value labels are centered above each bar.',
    '- Seat Pair Performance:',
    '  - For each seat entry: season cost, revenue, games sold, seats sold, balance (revenue - cost).',
    '- Insights:',
    '  - Average price per seat and pending payments count.',
    '- Clickables: none (display-only).',
    '',
    '## Events tab',
    '- Summary cards: Total Paid, Total Sold, Profit/Loss (= sold - paid).',
    '- Event list rows: name, date, paid/sold, status badge.',
    '- Clickables:',
    '  - Add Event button is present but currently not wired to an add flow.',
    '  - Trash icon deletes an event after confirmation.',
    '',
    '## Settings tab',
    '- Backup status banner (when present): shows last backup time and status; failed status includes Retry button.',
    '- Season Passes section:',
    '  - Add Season Pass opens Setup.',
    '  - Embed Logos toggle controls offline/self-contained backups.',
    '  - Edit Active Season Pass opens seat-entry editor.',
    '- Schedule section:',
    '  - Resync Schedule fetches latest home games (preserves sales).',
    '  - Import Schedule replaces schedule from CSV/Excel (preserves sales).',
    '- Data Management section:',
    '  - Export as Excel (.xlsx).',
    '  - Export as CSV (also copies detailed TSV rows to clipboard for easy Excel paste).',
    '  - Generate Clone Kit (creates shareable file + copies recovery code).',
    '  - Restore from Code (modal paste + restore; replaces current data).',
    '  - Restore from File (imports JSON backup; replaces current data).',
    '- Advanced / Troubleshooting section:',
    '  - Toggle show/hide advanced tools.',
    '  - Attempt ESPN Logos: tries to fill missing opponent logos.',
    '  - Restore All Season Pass Data: attempts recovery restore from stored backup.',
    '  - Replace Sales Data: paste seeded sales text to overwrite sales for the active pass.',
    '  - Force Replace Panthers Sales: wipes and replaces with canonical dataset.',
    '  - Email Backup and Export Backup to Folder: advanced sharing/export helpers.',
    '- Danger Zone:',
    '  - Delete Current Season Pass (active only).',
    '  - Clear All Data (all passes).',
    '',
    '## Edit Season Pass screen',
    '- Seat entry list sorted by section/row/seats.',
    '- Clickables:',
    '  - Add: opens modal to add a new seat entry.',
    '  - Pencil: edit seat entry.',
    '  - Trash: remove seat entry (confirm).',
    '',
    '## Restore screen',
    '- If opened with a `code` param, it auto-attempts restore after a short delay.',
    '- Clickables: Restore button attempts restore using the code.',
    '',
    '## Exports / backups (what each one is for)',
    '- Clone Kit: single text file containing Recovery Code + Backup JSON + this document.',
    '- Backup JSON: full data export used for file-based restore.',
    '- Recovery Code: compressed form of the same backup JSON (copy/paste restore).',
    '- Email Backup: shares a backup attachment intended to be email-client friendly (platform-dependent).',
    '- CSV export: per-game CSV; also copies detailed TSV rows to clipboard for Excel paste.',
    '- Excel export: `.xlsx` with Sales + Summary sheets.',
    '',
    '## Restore flows',
    '- Restore from Code: paste recovery code (or raw JSON) to replace current app data.',
    '- Restore from File: import backup JSON file to replace current app data.',
    '',
    '## Logos and “Embed Logos in Backup (Offline Restore)”',
    '- OFF: backups store logo URLs (logos can re-download later).',
    '- ON: backups embed logos as base64 `data:` URIs (offline-friendly, larger backups).',
    '',
    '## Math source (how totals are computed)',
    '- Revenue: sum of sale `price` values.',
    '- Seats per game: sum of parsed seat counts across seat entries.',
    '- Tickets sold: sums seatCount when present, otherwise parses from seats string, with a fallback default.',
    '- Sold rate: `ticketsSold / (gamesCount * seatsPerGame) * 100`.',
    '- Season cost: sum of seat entry `seasonCost` fields.',
    '',
  ].join('\n');

}

function buildCloneKitMarkdown(params: {
  createdAtISO: string;
  recoveryCode: string;
  backupJsonPretty: string;
  embedLogos: boolean;
}): string {
  const { createdAtISO, recoveryCode, backupJsonPretty, embedLogos } = params;
  const lines: string[] = [];
  lines.push('# Season Pass Manager — Clone Kit');
  lines.push('');
  lines.push(`Generated: ${createdAtISO}`);
  lines.push(`Logos embedded: ${embedLogos ? 'YES (offline-friendly)' : 'NO (logos may re-download when online)'}`);
  lines.push('');
  lines.push('## Quick restore (in a running copy of this app)');
  lines.push('1) Open Settings → Restore from Code');
  lines.push('2) Paste the Recovery Code from below');
  lines.push('3) Restore');
  lines.push('');
  lines.push('Alternative: Settings → Restore from File and select the JSON backup file.');
  lines.push('');
  lines.push('---');
  lines.push('');
  lines.push('## Recovery Code (copy/paste)');
  lines.push('');
  lines.push('```');
  lines.push(recoveryCode);
  lines.push('```');
  lines.push('');
  lines.push('---');
  lines.push('');
  lines.push('## Full Backup JSON (copy/paste)');
  lines.push('');
  lines.push('```json');
  lines.push(backupJsonPretty);
  lines.push('```');
  lines.push('');
  lines.push('---');
  lines.push('');
  lines.push(buildAppSnapshotMarkdown());
  lines.push('');
  return lines.join('\n');
}
...
function parseRecoveryCode(codeOrJson: string): BackupData | null {
  try {
    ...
    // If the input looks like JSON, parse directly
    if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
      const raw = JSON.parse(trimmed) as any;

      // Accept two shapes:
      // 1) BackupData (has .version and .seasonPasses)
      if (raw && raw.version && Array.isArray(raw.seasonPasses)) {
        return raw as BackupData;
      }

      // 2) Recovery context produced by catastrophic backup tools
      //    (contains recoveryData.salesData and recoveryData.seatPairs)
      if (raw && raw.recoveryData && (raw.recoveryData.salesData || raw.recoveryData.seatPairs)) {
        try {
          ...
          // Build a single Panthers season pass using the bundled schedule
          ...
          console.warn('[SeasonPass] parseRecoveryCode could not convert recoveryData shape:', inner);
          return null;
        }
      }
    ...
    // Otherwise assume it's the compressed recovery code
    const decompressed = LZString.decompressFromEncodedURIComponent(codeOrJson);
    if (!decompressed) return null;
    const parsed = JSON.parse(decompressed) as any;

    // If decompressed is already BackupData shape, return it
    if (parsed && parsed.version && Array.isArray(parsed.seasonPasses)) {
      return parsed as BackupData;
    }

    // If decompressed is a recovery context, convert it to BackupData
    if (parsed && parsed.recoveryData) {
      try {
        const rd = parsed.recoveryData;
        const createdAtISO = rd.timestamp || rd.lastBackup || new Date().toISOString();
        const seatPairs = rd.seatPairs || rd.seat_pairs || [];
        const salesRaw = rd.salesData || rd.sales_data || {};
        ...
        const backup: BackupData = {
          version: rd.version || parsed.version || '1.0',
          createdAtISO,
          activeSeasonPassId: passId,
          seasonPasses: [seasonPass],
        };
        ...
        console.warn('[SeasonPass] parseRecoveryCode failed converting decompressed recoveryData:', inner);
        return null;
      }
    }
...
// Standalone events (concerts / one-off games / any separate revenue), NOT tied to season passes.
const MISC_EVENTS_KEY = 'misc_events_v1';
const MISC_EVENTS_MIGRATED_KEY = 'misc_events_migrated_v1';

// Tracks the last time the user generated a restoreable backup artifact
// (Clone Kit / Backup JSON / Email Backup). This is separate from automatic
// internal snapshots written during normal app usage.
const LAST_USER_BACKUP_AT_ISO_KEY = 'last_user_backup_at_iso_v1';
...
async function safeSeedPanthersIfEmpty(): Promise<SeasonPass | null> {
  ...
  const seasonPass: SeasonPass = {
    ...
    salesData: transformSalesData(INITIAL_BACKUP_DATA.salesData),
    games: [],
    events: [],
...
export const [SeasonPassProvider, useSeasonPass] = createContextHook(() => {
  ...
  const seasonPassesRef = useRef<SeasonPass[]>([]);
  ...
  const [lastUserBackupAtISO, setLastUserBackupAtISO] = useState<string | null>(null);

  const [miscEvents, setMiscEvents] = useState<Event[]>([]);
  const miscEventsRef = useRef<Event[]>([]);
  useEffect(() => {
    miscEventsRef.current = miscEvents;
  }, [miscEvents]);

  const [cloudSyncEnabled, setCloudSyncEnabledState] = useState(false);
  const [cloudSyncKey, setCloudSyncKeyState] = useState<string>('');
  const [cloudSyncStatus, setCloudSyncStatus] = useState<'idle' | 'pushing' | 'pulling' | 'error'>('idle');
  const [cloudSyncError, setCloudSyncError] = useState<string | null>(null);
  const [lastCloudSyncTime, setLastCloudSyncTime] = useState<string | null>(null);

  const cloudSyncStatusRef = useRef<'idle' | 'pushing' | 'pulling' | 'error'>('idle');

  const [cloudSyncAutoUploadEnabled, setCloudSyncAutoUploadEnabledState] = useState(true);
  const [cloudSyncAutoDownloadEnabled, setCloudSyncAutoDownloadEnabledState] = useState(true);

  const cloudSyncLastServerUpdatedAtRef = useRef<string | null>(null);
  const cloudSyncLastLocalMutationAtRef = useRef<string | null>(null);
  const cloudSyncLastSyncedLocalMutationAtRef = useRef<string | null>(null);
  const cloudSyncAutoPushTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  // Track last schedule auto-fetch attempt per pass to prevent tight loops,
  // but still allow retries later (e.g. user created pass offline, comes online later).
  const fetchAttemptedRef = useRef<Map<string, number>>(new Map());
  const isInitialLoad = useRef(true);

  // Keep a ref to the latest passes so mutations can be computed synchronously.
  // This avoids relying on AsyncStorage reads and avoids missing persistence when
  // React batches state updates (common on web).
  useEffect(() => {
    seasonPassesRef.current = seasonPasses;
  }, [seasonPasses]);

  useEffect(() => {
    cloudSyncStatusRef.current = cloudSyncStatus;
  }, [cloudSyncStatus]);

  const setSeasonPassesWithRef = useCallback((next: SeasonPass[]) => {
    seasonPassesRef.current = next;
    setSeasonPasses(next);
  }, [lastUserBackupAtISO]);

  const setCloudSyncEnabled = useCallback(async (enabled: boolean) => {
    setCloudSyncEnabledState(enabled);
    // Clear any stale error/status when toggling. Backend availability can change
    // between runs (e.g., Expo tunnel vs LAN), so old errors are misleading.
    setCloudSyncError(null);
    setCloudSyncStatus('idle');
    try {
      await AsyncStorage.setItem(CLOUD_SYNC_ENABLED_KEY, JSON.stringify(enabled));
    } catch (e) {
      console.warn('[CloudSync] Failed to persist enabled flag', e);
    }
  }, []);

  const setCloudSyncAutoUploadEnabled = useCallback(async (enabled: boolean) => {
    setCloudSyncAutoUploadEnabledState(enabled);
    try {
      await AsyncStorage.setItem(CLOUD_SYNC_AUTO_UPLOAD_ENABLED_KEY, JSON.stringify(enabled));
    } catch (e) {
      console.warn('[CloudSync] Failed to persist auto-upload flag', e);
    }
  }, []);

  const setCloudSyncAutoDownloadEnabled = useCallback(async (enabled: boolean) => {
    setCloudSyncAutoDownloadEnabledState(enabled);
    try {
      await AsyncStorage.setItem(CLOUD_SYNC_AUTO_DOWNLOAD_ENABLED_KEY, JSON.stringify(enabled));
    } catch (e) {
      console.warn('[CloudSync] Failed to persist auto-download flag', e);
    }
  }, []);

  const setCloudSyncKey = useCallback(async (key: string) => {
    const normalized = String(key || '').trim();
    setCloudSyncKeyState(normalized);
    try {
      await AsyncStorage.setItem(CLOUD_SYNC_SECRET_KEY, normalized);
    } catch (e) {
      console.warn('[CloudSync] Failed to persist key', e);
    }
  }, []);

  const generateCloudSyncKey = useCallback(async (): Promise<string> => {
    // Not cryptographically perfect, but fine for a shared "pairing" secret.
    const alphabet = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
    let out = '';
    for (let i = 0; i < 28; i += 1) {
      out += alphabet[Math.floor(Math.random() * alphabet.length)];
      if (i === 6 || i === 13 || i === 20) out += '-';
    }
    await setCloudSyncKey(out);
    return out;
  }, [setCloudSyncKey]);

  const loadCloudSyncSettings = useCallback(async () => {
    try {
      const enabledRaw = await AsyncStorage.getItem(CLOUD_SYNC_ENABLED_KEY);
      const keyRaw = await AsyncStorage.getItem(CLOUD_SYNC_SECRET_KEY);
      const lastServerRaw = await AsyncStorage.getItem(CLOUD_SYNC_LAST_SERVER_UPDATED_AT_KEY);
      const autoUploadRaw = await AsyncStorage.getItem(CLOUD_SYNC_AUTO_UPLOAD_ENABLED_KEY);
      const autoDownloadRaw = await AsyncStorage.getItem(CLOUD_SYNC_AUTO_DOWNLOAD_ENABLED_KEY);
      const lastLocalMutationRaw = await AsyncStorage.getItem(CLOUD_SYNC_LAST_LOCAL_MUTATION_AT_KEY);
      const lastSyncedLocalMutationRaw = await AsyncStorage.getItem(CLOUD_SYNC_LAST_SYNCED_LOCAL_MUTATION_AT_KEY);

      const enabled = enabledRaw ? Boolean(JSON.parse(enabledRaw)) : false;
      const key = (keyRaw || '').trim();
      const lastServer = lastServerRaw ? String(lastServerRaw) : null;
      const autoUpload = autoUploadRaw ? Boolean(JSON.parse(autoUploadRaw)) : true;
      const autoDownload = autoDownloadRaw ? Boolean(JSON.parse(autoDownloadRaw)) : true;
      const lastLocalMutationAt = lastLocalMutationRaw ? String(lastLocalMutationRaw) : null;
      const lastSyncedLocalMutationAt = lastSyncedLocalMutationRaw ? String(lastSyncedLocalMutationRaw) : null;

      setCloudSyncEnabledState(enabled);
      setCloudSyncKeyState(key);
      cloudSyncLastServerUpdatedAtRef.current = lastServer;
      setCloudSyncAutoUploadEnabledState(autoUpload);
      setCloudSyncAutoDownloadEnabledState(autoDownload);
      cloudSyncLastLocalMutationAtRef.current = lastLocalMutationAt;
      cloudSyncLastSyncedLocalMutationAtRef.current = lastSyncedLocalMutationAt;
    } catch (e) {
      console.warn('[CloudSync] Failed to load settings', e);
    }
  }, []);

  const markLocalMutationNow = useCallback(async () => {
    const now = new Date().toISOString();
    cloudSyncLastLocalMutationAtRef.current = now;
    try {
      await AsyncStorage.setItem(CLOUD_SYNC_LAST_LOCAL_MUTATION_AT_KEY, now);
    } catch {
      // ignore
    }
    return now;
  ...
  const markLocalSyncedMutationAt = useCallback(async (mutationAtISO: string) => {
    cloudSyncLastSyncedLocalMutationAtRef.current = mutationAtISO;
    try {
      await AsyncStorage.setItem(CLOUD_SYNC_LAST_SYNCED_LOCAL_MUTATION_AT_KEY, mutationAtISO);
    } catch {
      // ignore
    }
  }, []);

  const pushCloudSyncNow = useCallback(async (): Promise<{ success: boolean; error?: string }> => {
    if (!cloudSyncEnabled || !cloudSyncKey) return { success: false, error: 'Cloud sync not configured.' };

    const resolved = resolveApiBaseUrl();
    if (!resolved.baseUrl) {
      const msg = __DEV__
        ? 'Cloud sync backend is not available in this run. Start the local API (`bun run start:api`) and keep your phone on the same Wi‑Fi, or set EXPO_PUBLIC_RORK_API_BASE_URL to a reachable server URL.'
        : 'Sync service is not available right now. Please try again later.';
    ...
    try {
      ...
      const masterRaw = await AsyncStorage.getItem(MASTER_BACKUP_KEY);
      if (!masterRaw) {
        setCloudSyncStatus('idle');
        return { success: false, error: 'No master backup found to sync.' };
      }

      const res = await trpcClient.sync.putBackup.mutate({
        key: cloudSyncKey,
        backupJson: masterRaw,
      });

      cloudSyncLastServerUpdatedAtRef.current = res.serverUpdatedAtISO;
      await AsyncStorage.setItem(CLOUD_SYNC_LAST_SERVER_UPDATED_AT_KEY, res.serverUpdatedAtISO);
      setLastCloudSyncTime(new Date().toLocaleString());
      // Mark local changes as synced (best-effort).
      const localMutationAt = cloudSyncLastLocalMutationAtRef.current;
      if (localMutationAt) {
        await markLocalSyncedMutationAt(localMutationAt);
      }
      setCloudSyncStatus('idle');
      return { success: true };
    } catch (e: any) {
      const msg = e?.message || 'Cloud sync push failed.';
      console.warn('[CloudSync] Push failed:', msg);
      setCloudSyncError(`${msg}${resolved.baseUrl ? ` (base: ${resolved.baseUrl})` : ''}`);
      setCloudSyncStatus('idle');
      return { success: false, error: msg };
    }
  }, [cloudSyncEnabled, cloudSyncKey, markLocalSyncedMutationAt]);

  const scheduleAutoPush = useCallback((reason: string) => {
    if (!cloudSyncEnabled || !cloudSyncKey || !cloudSyncAutoUploadEnabled) return;
    if (cloudSyncAutoPushTimerRef.current) {
      clearTimeout(cloudSyncAutoPushTimerRef.current);
      cloudSyncAutoPushTimerRef.current = null;
    }
    cloudSyncAutoPushTimerRef.current = setTimeout(async () => {
      cloudSyncAutoPushTimerRef.current = null;
      // Only push if there are unsynced local changes.
      const local = cloudSyncLastLocalMutationAtRef.current;
      const synced = cloudSyncLastSyncedLocalMutationAtRef.current;
      if (!local || local === synced) return;
      if (cloudSyncStatusRef.current !== 'idle') return;

      console.log('[CloudSync] Auto-upload starting (reason:', reason + ')');
      const res = await pushCloudSyncNow();
      if (res.success) {
        await markLocalSyncedMutationAt(local);
      }
    }, 1500);
  }, [cloudSyncEnabled, cloudSyncKey, cloudSyncAutoUploadEnabled, pushCloudSyncNow, markLocalSyncedMutationAt]);

  const pullCloudSyncNow = useCallback(async (): Promise<{ success: boolean; updated: boolean; exists: boolean; error?: string }> => {
    if (!cloudSyncEnabled || !cloudSyncKey) return { success: false, updated: false, exists: false, error: 'Cloud sync not configured.' };
    ...
    if (!resolved.baseUrl) {
      const msg = __DEV__
        ...
        : 'Sync service is not available right now. Please try again later.';
      setCloudSyncError(msg);
      return { success: false, updated: false, exists: false, error: msg };
    }
    ...
    try {
      ...
      const meta = await trpcClient.sync.getMeta.query({ key: cloudSyncKey });
      if (!meta.exists) {
        setCloudSyncStatus('idle');
        return { success: true, updated: false, exists: false };
      }

      const lastServer = cloudSyncLastServerUpdatedAtRef.current;
      if (lastServer && meta.serverUpdatedAtISO === lastServer) {
        setCloudSyncStatus('idle');
        return { success: true, updated: false, exists: true };
      }

      const backupRes = await trpcClient.sync.getBackup.query({ key: cloudSyncKey });
      if (!backupRes?.backupJson) {
        setCloudSyncStatus('idle');
        return { success: true, updated: false, exists: true };
      }

      const parsed = JSON.parse(backupRes.backupJson) as any;
      if (!parsed || !Array.isArray(parsed.seasonPasses)) {
        throw new Error('Remote backup is invalid (missing seasonPasses).');
      }

      const restoredMiscEvents: Event[] = Array.isArray(parsed.miscEvents) ? (parsed.miscEvents as Event[]) : [];
      const restoredLastUserBackupAtISO: string | null = typeof parsed.lastUserBackupAtISO === 'string' ? String(parsed.lastUserBackupAtISO) : null;

      const restoredPasses: SeasonPass[] = (parsed.seasonPasses as any[]).map(normalizeSeasonPass);
      const restoredActiveId: string | null = parsed.activeSeasonPassId && restoredPasses.find(p => p.id === parsed.activeSeasonPassId)
        ? parsed.activeSeasonPassId
        : (restoredPasses[0]?.id || null);

      // Persist exactly what we pulled.
      await AsyncStorage.setItem(SEASON_PASSES_KEY, JSON.stringify(restoredPasses));
      await AsyncStorage.setItem(ACTIVE_PASS_KEY, JSON.stringify(restoredActiveId));
      await AsyncStorage.setItem(DATA_IMPORTED_KEY, 'true');
      await AsyncStorage.setItem(MISC_EVENTS_KEY, JSON.stringify(restoredMiscEvents));
      if (restoredLastUserBackupAtISO) {
        await AsyncStorage.setItem(LAST_USER_BACKUP_AT_ISO_KEY, restoredLastUserBackupAtISO);
      } else {
        await AsyncStorage.removeItem(LAST_USER_BACKUP_AT_ISO_KEY);
      }
      await AsyncStorage.setItem(MASTER_BACKUP_KEY, JSON.stringify({
        version: parsed.version || BACKUP_VERSION,
        createdAtISO: parsed.createdAtISO || new Date().toISOString(),
        activeSeasonPassId: restoredActiveId,
        seasonPasses: restoredPasses,
        miscEvents: restoredMiscEvents,
        lastUserBackupAtISO: restoredLastUserBackupAtISO,
      }));
      await AsyncStorage.setItem(ALL_PASSES_BACKUP_KEY, JSON.stringify({
        timestamp: new Date().toISOString(),
        passCount: restoredPasses.length,
        passes: restoredPasses,
      }));

      setSeasonPassesWithRef(restoredPasses);
      setActiveSeasonPassId(restoredActiveId);
      setMiscEvents(restoredMiscEvents);
      setLastUserBackupAtISO(restoredLastUserBackupAtISO);
      setSalesDataVersion(v => v + 1);

      cloudSyncLastServerUpdatedAtRef.current = backupRes.serverUpdatedAtISO;
      await AsyncStorage.setItem(CLOUD_SYNC_LAST_SERVER_UPDATED_AT_KEY, backupRes.serverUpdatedAtISO);
      setLastCloudSyncTime(new Date().toLocaleString());
      // Pull replaces local state; mark local as synced at this point.
      const now = new Date().toISOString();
      cloudSyncLastLocalMutationAtRef.current = now;
      await AsyncStorage.setItem(CLOUD_SYNC_LAST_LOCAL_MUTATION_AT_KEY, now);
      await markLocalSyncedMutationAt(now);
      setCloudSyncStatus('idle');
      return { success: true, updated: true, exists: true };
    } catch (e: any) {
      const msg = e?.message || 'Cloud sync pull failed.';
      console.warn('[CloudSync] Pull failed:', msg);
      setCloudSyncError(`${msg}${resolved.baseUrl ? ` (base: ${resolved.baseUrl})` : ''}`);
      setCloudSyncStatus('idle');
      return { success: false, updated: false, exists: true, error: msg };
    }
  }, [cloudSyncEnabled, cloudSyncKey, setSeasonPassesWithRef, markLocalSyncedMutationAt]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      let hasStoredMiscEvents = false;

      // Load the last user-generated backup timestamp (Clone Kit / Backup JSON).
      try {
        const lastUserBackupIsoRaw = await AsyncStorage.getItem(LAST_USER_BACKUP_AT_ISO_KEY);
        if (lastUserBackupIsoRaw && typeof lastUserBackupIsoRaw === 'string') {
          setLastUserBackupAtISO(lastUserBackupIsoRaw);
        }
      } catch (e) {
        console.warn('[SeasonPass] Failed to load last user backup timestamp', e);
      }

      // Load standalone Misc Events (separate from season pass data).
      try {
        const miscRaw = await AsyncStorage.getItem(MISC_EVENTS_KEY);
        if (miscRaw) {
          const parsed = JSON.parse(miscRaw);
          if (Array.isArray(parsed)) {
            setMiscEvents(parsed as Event[]);
            hasStoredMiscEvents = parsed.length > 0;
          }
        }
      } catch (e) {
        console.warn('[SeasonPass] Failed to load misc events', e);
      }
  ...
  // Migration: remove any invalid or $0 sale records.
  // This helps prevent phantom "sold for $0" entries from reappearing after restores.
  const migrateRemoveZeroPriceSales = (passList: SeasonPass[]) => {
    let changed = false;
    let removedCount = 0;
    ...
    const migrated = passList.map(p => {
      ...
      Object.entries(sales).forEach(([gameId, gameSales]: any) => {
        const nextGameSales: Record<string, any> = { ...(gameSales || {}) };
        Object.entries(nextGameSales).forEach(([pairId, sale]: any) => {
          if (!sale) return;
          const price = sale.price;
          if (typeof price !== 'number' || !Number.isFinite(price) || price <= 0) {
            delete nextGameSales[pairId];
            removedCount += 1;
            changed = true;
          }
        });
        // Persist explicit empty object for "zero sales" games
        sales[gameId] = Object.keys(nextGameSales).length === 0 ? {} : nextGameSales;
      });

      pClone.salesData = sales;
      return pClone as SeasonPass;
    });
  ...
  const zeroMigration = migrateRemoveZeroPriceSales(passes);
  if (zeroMigration.changed) {
    console.log('[SeasonPass] Migration applied: removed invalid/$0 sale records:', zeroMigration.removedCount);
    passes = zeroMigration.migrated;
    try {
      await AsyncStorage.setItem(SEASON_PASSES_KEY, JSON.stringify(passes));
      console.log('[SeasonPass] Zero-price sale cleanup saved to storage');
    } catch (e: any) {
      console.warn('[SeasonPass] Failed to persist zero-price sale cleanup', e);
    }
  }

  // Migration: move legacy per-season-pass events into standalone misc events.
  // Only runs once, and only when there are no stored misc events yet.
  try {
    const migratedFlag = await AsyncStorage.getItem(MISC_EVENTS_MIGRATED_KEY);
    if (!migratedFlag) {
      ...
      const nextPasses = passes.map((p) => {
        if (p.events && p.events.length > 0) {
          ...
          p.events.forEach((ev) => {
            legacyEvents.push({
              ...ev,
              id: `migr_${p.id}_${ev.id || Date.now()}`,
            });
          });
          return normalizeSeasonPass({ ...p, events: [] });
        }
        return p;
      });

      if (anyLegacyFound && !hasStoredMiscEvents) {
        console.log('[SeasonPass] Migrating legacy per-pass events to standalone misc events:', legacyEvents.length);
        await AsyncStorage.setItem(MISC_EVENTS_KEY, JSON.stringify(legacyEvents));
        setMiscEvents(legacyEvents);
        passes = nextPasses;
        await AsyncStorage.setItem(SEASON_PASSES_KEY, JSON.stringify(passes));
      }

      await AsyncStorage.setItem(MISC_EVENTS_MIGRATED_KEY, 'true');
    }
  } catch (e: any) {
    console.warn('[SeasonPass] Failed to migrate legacy events to misc events', e);
  }
  ...
  setSeasonPasses(passes);
      ...
      // AUTO-RESTORE: Check for injected accurate data
      try {
        const autoRestoreCompleted = await AsyncStorage.getItem(AUTO_RESTORE_FLAG);
        if (false && !autoRestoreCompleted) {
          console.log('[SeasonPass] Checking for auto-restore data...');
          // Embedded recovery code from seasonpass_restore_panthers_2025.json (UPDATED: Game IDs shifted for new Tampa game on Dec 27)
          const RECOVERY_CODE = "";
          
          if (RECOVERY_CODE) {
            console.log('[SeasonPass] 🔄 Auto-restoring accurate sales data...');
            try {
              const decompressed = LZString.decompressFromBase64(RECOVERY_CODE);
              if (!decompressed) throw new Error('Decompression failed');
              
              const backupData = JSON.parse(decompressed);
              if (backupData && backupData.seasonPasses) {
                const restoredPasses = backupData.seasonPasses.map((p: any) => normalizeSeasonPass(p));
                await AsyncStorage.setItem(SEASON_PASSES_KEY, JSON.stringify(restoredPasses));
                await AsyncStorage.setItem(ACTIVE_PASS_KEY, backupData.activeSeasonPassId || restoredPasses[0]?.id || '');
                await AsyncStorage.setItem(AUTO_RESTORE_FLAG, 'true');
                setSeasonPasses(restoredPasses);
                setActiveSeasonPassId(backupData.activeSeasonPassId || restoredPasses[0]?.id || null);
                console.log('[SeasonPass] ✅ Auto-restore completed successfully!');
                passes = restoredPasses; // Update passes for migration below
              }
            } catch (restoreError) {
              console.error('[SeasonPass] Auto-restore parsing failed:', restoreError);
            }
          }
        } else {
          console.log('[SeasonPass] Auto-restore already completed, skipping...');
        }
      } catch (autoRestoreError) {
        console.error('[SeasonPass] Auto-restore error:', autoRestoreError);
      }
  ...
  const saveMiscEvents = useCallback(async (next: Event[]) => {
    setMiscEvents(next);
    try {
      await AsyncStorage.setItem(MISC_EVENTS_KEY, JSON.stringify(next));
    } catch (e) {
      console.warn('[SeasonPass] Failed to persist misc events', e);
    }

    // Keep master backup updated so exports + cloud sync include misc events.
    try {
      const dataImportedRaw = await AsyncStorage.getItem(DATA_IMPORTED_KEY);
      const masterBackup: BackupData = {
        ...
        seasonPasses: seasonPassesRef.current,
        miscEvents: next,
        lastUserBackupAtISO: lastUserBackupAtISO ?? null,
      ...
      console.warn('[SeasonPass] Failed to update master backup for misc events', e);
    }

    // Treat misc-event changes as a local mutation for sync safety.
    try {
      const localMutationAt = await markLocalMutationNow();
      if (cloudSyncEnabled && cloudSyncKey && cloudSyncAutoUploadEnabled) {
        if (localMutationAt !== cloudSyncLastSyncedLocalMutationAtRef.current) {
          scheduleAutoPush('misc-events');
        }
      }
    } catch {
      // ignore
    ...
    // Mirror the normal backup confirmation UX.
  ...
  }, [activeSeasonPassId, cloudSyncAutoUploadEnabled, cloudSyncEnabled, cloudSyncKey, lastUserBackupAtISO, markLocalMutationNow, scheduleAutoPush]);

  const addMiscEvent = useCallback(async (event: Event) => {
    const next = [event, ...miscEventsRef.current];
    await saveMiscEvents(next);
  }, [saveMiscEvents]);

  const updateMiscEvent = useCallback(async (eventId: string, updates: Partial<Event>) => {
    const next = miscEventsRef.current.map((ev) => (ev.id === eventId ? { ...ev, ...updates } : ev));
    await saveMiscEvents(next);
  }, [saveMiscEvents]);

  const removeMiscEvent = useCallback(async (eventId: string) => {
    const next = miscEventsRef.current.filter((ev) => ev.id !== eventId);
    await saveMiscEvents(next);
  }, [saveMiscEvents]);

  const markUserBackupNow = useCallback(async (): Promise<string> => {
    const iso = new Date().toISOString();
    setLastUserBackupAtISO(iso);
    try {
      await AsyncStorage.setItem(LAST_USER_BACKUP_AT_ISO_KEY, iso);
    } catch (e) {
      console.warn('[SeasonPass] Failed to persist last user backup timestamp', e);
    }
    return iso;
  }, []);
  ...
  const fillOpponentLogosForLeague = (games: Game[], leagueId?: string): Game[] => {
    if (!Array.isArray(games) || !leagueId) {
      console.log('[FillLogos] Skipping - games array invalid or no leagueId');
      return games;
    }
    try {
      const teams = getTeamsByLeague(leagueId) || [];
      console.log(`[FillLogos] Processing ${games.length} games for league ${leagueId}, found ${teams.length} teams`);
      
      let filledCount = 0;
      const result = games.map((g, idx) => {
        if (g.opponentLogo) {
          console.log(`[FillLogos] Game ${idx + 1}: "${g.opponent}" already has logo`);
          return g;
        }
        
        const opp = (g.opponent || '').trim();
        console.log(`[FillLogos] Game ${idx + 1}: Searching logo for opponent "${opp}"`);
        
        if (!opp) {
          console.log(`[FillLogos] Game ${idx + 1}: Empty opponent name, skipping`);
          return g;
        }
        ...
        if (direct && direct.logoUrl) {
          console.log(`[FillLogos] Game ${idx + 1}: ✅ Found logo via direct match: ${direct.name}`);
          filledCount++;
          return { ...g, opponentLogo: direct.logoUrl };
        }

        // Try token-based matching
        const txt = opp.toLowerCase().replace(/[^a-z0-9\s]/g, ' ');
        const tokens: string[] = txt.split(/\s+/).filter(Boolean);
        console.log(`[FillLogos] Game ${idx + 1}: Trying tokens: ${tokens.join(', ')}`);
        ...
        for (const t of teams) {
          ...
          for (const token of tokens) {
            if (name.includes(token) || city.includes(token) || (t.abbreviation || '').toLowerCase() === token) {
              console.log(`[FillLogos] Game ${idx + 1}: ✅ Found logo via token "${token}": ${t.name}`);
              filledCount++;
      ...
      console.log(`[FillLogos] Filled opponent logos: ${filledCount}/${games.length}`);
      return result;
    } catch {
      return games;
    }
  ...
  const saveSeasonPasses = useCallback(async (passes: SeasonPass[]) => {
    try {
      ...
      if (passes.length > 0) {
        await saveAllPassesBackup(passes);
        try {
          // Show on-screen confirmation for every save+backup (requirement: confirmation on every input change)
          Alert.alert('Backup updated', 'Backup updated successfully ✅');
        } catch (err) {
          // In non-native environments Alert may not be available; ignore
        }
      }
    } catch (error) {
  ...
  const performAutoBackup = useCallback(async (passes: SeasonPass[], activeId: string | null): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      const masterBackup: BackupData = {
        ...
        seasonPasses: passes,
        miscEvents: miscEventsRef.current,
        lastUserBackupAtISO: lastUserBackupAtISO ?? null,
      };
      await AsyncStorage.setItem(MASTER_BACKUP_KEY, JSON.stringify(masterBackup));
      console.log('[AutoBackup] Master backup saved');

      // Track that local state changed (used to decide safe auto-pull and whether auto-upload is needed).
      const localMutationAt = await markLocalMutationNow();

      // Cloud sync: optional auto-upload (debounced). Never block local save.
      if (cloudSyncEnabled && cloudSyncKey && cloudSyncAutoUploadEnabled) {
        // Only schedule if this change isn't already marked as synced.
        if (localMutationAt !== cloudSyncLastSyncedLocalMutationAtRef.current) {
          scheduleAutoPush('auto-backup');
        }
      }
  ...
  }, [cloudSyncEnabled, cloudSyncKey, cloudSyncAutoUploadEnabled, lastUserBackupAtISO, markLocalMutationNow, scheduleAutoPush]);

  // Load cloud sync settings on startup.
  useEffect(() => {
    loadCloudSyncSettings();
  }, [loadCloudSyncSettings]);

  // Cloud sync auto-download: periodically pull only when it's safe (no unsynced local changes).
  useEffect(() => {
    if (!cloudSyncEnabled || !cloudSyncKey || !cloudSyncAutoDownloadEnabled) return;

    let cancelled = false;
    const safeAutoPull = async () => {
      if (cancelled) return;
      if (cloudSyncStatusRef.current !== 'idle') return;

      const local = cloudSyncLastLocalMutationAtRef.current;
      const synced = cloudSyncLastSyncedLocalMutationAtRef.current;
      // If local has changed since last successful sync, do not auto-pull.
      if (local && synced && local !== synced) return;

      await pullCloudSyncNow();
    };

    safeAutoPull();
    const id = setInterval(safeAutoPull, 30000);
    return () => {
      cancelled = true;
      clearInterval(id);
    };
  }, [cloudSyncEnabled, cloudSyncKey, cloudSyncAutoDownloadEnabled, pullCloudSyncNow]);
  ...
  const updateSeasonPass = useCallback(async (passId: string, updates: Partial<SeasonPass>) => {
    const currentPasses = seasonPassesRef.current;
    const updatedPasses = currentPasses.map(sp => (sp.id === passId ? { ...sp, ...updates } : sp));
    setSeasonPassesWithRef(updatedPasses);
  ...
  }, [activeSeasonPassId, fetchScheduleWithMasterTimeout, performAutoBackup, setSeasonPassesWithRef]);
  ...
  const updateSeatPair = useCallback(async (
    passId: string,
    seatPairId: string,
    updates: Partial<SeatPair>
  ): Promise<{ success: boolean; error?: string }> => {
    try {
      const currentPasses = seasonPassesRef.current;
      const pass = currentPasses.find(p => p.id === passId);
      if (!pass) return { success: false, error: 'Season pass not found.' };
      const existing = pass.seatPairs?.find(p => p.id === seatPairId);
      if (!existing) return { success: false, error: 'Seat entry not found.' };

      const updatedPasses = currentPasses.map(p => {
        if (p.id !== passId) return p;
        const nextSeatPairs = (p.seatPairs || []).map(sp => {
          if (sp.id !== seatPairId) return sp;
          const next: SeatPair = { ...sp, ...updates };
          if (typeof (next as any).seasonCost !== 'number' || Number.isNaN((next as any).seasonCost)) {
            next.seasonCost = sp.seasonCost;
          }
          if (next.seasonCost < 0) next.seasonCost = 0;
          return next;
        });
        return { ...p, seatPairs: nextSeatPairs };
      });

      setSeasonPassesWithRef(updatedPasses);
      const backupResult = await performAutoBackup(updatedPasses, activeSeasonPassId);
      console.log('[SeasonPass] Updated seat pair - Backup:', backupResult.success ? 'SUCCESS' : 'FAILED');
      return { success: true };
    } catch (e: any) {
      console.error('[SeasonPass] updateSeatPair failed:', e);
      return { success: false, error: e?.message || 'Failed to update seat.' };
    }
  }, [activeSeasonPassId, performAutoBackup, setSeasonPassesWithRef]);
  ...
  const addSaleRecord = useCallback(async (
    ...
    const pass = seasonPasses.find(sp => sp.id === passId);
    if (pass) {
      // Defensive: do not persist $0 / invalid sales.
      // These can reappear via restore/merge and skew Seats Sold / Pending.
      const price = (saleRecord as any)?.price;
      if (typeof price !== 'number' || !Number.isFinite(price) || price <= 0) {
        const pairId = (saleRecord as any)?.pairId;
        console.warn('[SeasonPass] Ignoring non-positive/invalid sale price; removing existing sale if present', {
          passId,
          gameId,
          pairId,
          price,
        ...
        if (pairId && pass.salesData?.[gameId]?.[pairId]) {
          const updatedSalesData = { ...pass.salesData };
          const gameSales = { ...(updatedSalesData[gameId] || {}) };
          delete gameSales[pairId];
          updatedSalesData[gameId] = Object.keys(gameSales).length === 0 ? {} : gameSales;
  ...
  const removeSaleRecord = useCallback(async (passId: string, gameId: string, pairId: string) => {
    ...
    delete gameSales[pairId];
    // Persist zero-sales state: keep an empty object for the game instead of removing
    if (Object.keys(gameSales).length === 0) {
      // keep an explicit empty object so the game persists with zero sales
      updatedSalesData[gameId] = {};
    } else {
  ...
  // Remove all sales for a specific game but persist an explicit empty sales object
  const removeAllSalesForGame = useCallback(async (passId: string, gameId: string) => {
    const pass = seasonPasses.find(sp => sp.id === passId);
    if (!pass) return;

    const updatedSalesData = { ...pass.salesData };
    // set to explicit empty object to represent zero sales
    updatedSalesData[gameId] = {};

    await updateSeasonPass(passId, { salesData: updatedSalesData });
    console.log('[SeasonPass] Removed all sales for game', gameId, 'in pass', passId);
  }, [seasonPasses, updateSeasonPass]);
  ...
  const resyncAllSchedules = useCallback(async (): Promise<{
    success: boolean;
    updatedCount: number;
    failedCount: number;
    failures?: Array<{ passId: string; teamName: string; error: string }>;
    error?: string;
  }> => {
    const passes = seasonPassesRef.current;
    if (!Array.isArray(passes) || passes.length === 0) {
      return { success: false, updatedCount: 0, failedCount: 0, error: 'No season passes found.' };
    }

    setIsLoadingSchedule(true);
    setLastScheduleError(null);

    const failures: Array<{ passId: string; teamName: string; error: string }> = [];
    let updatedCount = 0;

    try {
      const updatedPasses: SeasonPass[] = [];

      for (const pass of passes) {
        try {
          // Panthers: use bundled schedule, no backend dependency.
          if (String(pass.leagueId).toLowerCase() === 'nhl' && String(pass.teamId).toLowerCase() === 'fla') {
            updatedPasses.push(normalizeSeasonPass({ ...pass, games: PANTHERS_20252026_SCHEDULE }));
            updatedCount += 1;
            continue;
          }
          ...
          if (result.games && result.games.length > 0) {
            ...
            // Backfill opponentLogo into any existing sales for the updated games
            const updatedSalesData: any = { ...(pass.salesData || {}) };
            for (const g of gamesWithLogos) {
              if (!g.opponentLogo) continue;
              const gid = g.id;
              const bySeat = updatedSalesData[gid];
            ...
            updatedPasses.push(normalizeSeasonPass({ ...pass, games: gamesWithLogos, salesData: updatedSalesData }));
            updatedCount += 1;
          } else {
            failures.push({
              passId: pass.id,
              teamName: pass.teamName,
              error: result.error ? String(result.error) : 'No games returned',
            });
            updatedPasses.push(normalizeSeasonPass(pass));
          }
        } catch (e: any) {
          failures.push({
            passId: pass.id,
            teamName: pass.teamName,
            error: e?.message || 'Schedule fetch failed',
          });
          updatedPasses.push(normalizeSeasonPass(pass));
        }
      }
      ...
      await AsyncStorage.setItem(SEASON_PASSES_KEY, JSON.stringify(updatedPasses));
      setSeasonPassesWithRef(updatedPasses);
      await performAutoBackup(updatedPasses, activeSeasonPassId);
      ...
      const msg = e?.message || 'Failed to resync schedules.';
      setLastScheduleError(msg);
      return { success: false, updatedCount: 0, failedCount: 0, error: msg };
    } finally {
      setIsLoadingSchedule(false);
    }
  }, [activeSeasonPassId, performAutoBackup, setSeasonPassesWithRef]);
  ...
  useEffect(() => {
    // If the active pass has no games yet (created offline, restore missing schedule, etc),
    // keep trying periodically until it loads — but with a cooldown to avoid loops.
    if (!activeSeasonPass) return;
    if (activeSeasonPass.games && activeSeasonPass.games.length > 0) return;
    ...
    const tick = async () => {
      ...
      const current = seasonPassesRef.current.find(p => p.id === passId) || activeSeasonPass;
      if (current.games && current.games.length > 0) return;

      const now = Date.now();
      const lastAttemptAt = fetchAttemptedRef.current.get(passId) ?? 0;
      if (now - lastAttemptAt < COOLDOWN_MS) return;

      console.log('[SeasonPass] Auto-fetching schedule for pass:', passId);
      fetchAttemptedRef.current.set(passId, now);
      await loadScheduleIfNeeded(current);
    };
  ...
  }, [activeSeasonPass?.id, activeSeasonPass?.games?.length, isLoadingSchedule, loadScheduleIfNeeded]);
  ...
  const clearAllData = useCallback(async () => {
    ...
    setNeedsSetup(true);
    setMiscEvents([]);
    await AsyncStorage.multiRemove([SEASON_PASSES_KEY, ACTIVE_PASS_KEY, MISC_EVENTS_KEY]);
    console.log('[SeasonPass] Cleared all data');
  }, []);
  ...
  const generateBackup = useCallback(async (embedLogos = false): Promise<BackupData> => {
    try {
      ...
      const lastUserBackupAtISOValue = lastUserBackupIsoRaw && typeof lastUserBackupIsoRaw === 'string'
        ? String(lastUserBackupIsoRaw)
        : (lastUserBackupAtISO ?? null);

      let miscEventsList: Event[] = miscEvents;
      if (miscRaw) {
        try {
          const parsedMisc = JSON.parse(miscRaw);
          if (Array.isArray(parsedMisc)) miscEventsList = parsedMisc as Event[];
        } catch {
          // ignore and use in-memory miscEvents
          miscEventsList = miscEvents;
        }
      }
      ...
      if (embedLogos) {
        try {
          ...
          return {
            ...
            miscEvents: miscEventsList,
            lastUserBackupAtISO: lastUserBackupAtISOValue,
            dataImportedRaw: dataImportedRaw ?? null,
            appTheme: AppColors ?? null,
          };
        } catch (embedErr) {
  ...
  }, [seasonPasses, activeSeasonPassId, miscEvents, lastUserBackupAtISO]);
  ...
  const createRecoveryCode = useCallback(async (embedLogos = false): Promise<string> => {
    try {
      ...
      // Build a single "Clone Kit" markdown file (recovery code + JSON + UI snapshot)
      const backupJsonPretty = JSON.stringify(backup, null, 2);
      const createdAtISO = backup.createdAtISO || new Date().toISOString();
      const cloneKitMd = buildCloneKitMarkdown({
        createdAtISO,
        recoveryCode: code,
        backupJsonPretty,
        embedLogos,
      ...
      try {
        const jsonString = backupJsonPretty;
        const dateStr = new Date().toISOString().split('T')[0];
        const fileName = `SeasonPassBackup_${dateStr}.json`;

        const cloneKitName = `SeasonPassCloneKit_${dateStr}.md`;
        ...
        if (Platform.OS === 'web') {
          ...
          a.click();
          URL.revokeObjectURL(url);
          // Also download the Clone Kit markdown for easy copy/paste + email
          const kitBlob = new Blob([cloneKitMd], { type: 'text/markdown' });
          const ka = document.createElement('a');
          const kitUrl = URL.createObjectURL(kitBlob);
          ka.href = kitUrl;
          ka.download = cloneKitName;
          ka.click();
          ...
          await markUserBackupNow();
          return fileName;
        } else {
          const fileUri = FileSystem.documentDirectory + fileName;
          await FileSystem.writeAsStringAsync(fileUri, jsonString, { encoding: FileSystem.EncodingType.UTF8 });

          const cloneKitUri = FileSystem.documentDirectory + cloneKitName;
          await FileSystem.writeAsStringAsync(cloneKitUri, cloneKitMd, { encoding: FileSystem.EncodingType.UTF8 });
          ...
          try {
            ...
            // Share the Clone Kit markdown (single attachment that includes code + JSON + docs)
            if (await Sharing.isAvailableAsync()) {
              await Sharing.shareAsync(cloneKitUri, {
                mimeType: 'text/plain',
                dialogTitle: 'Share Clone Kit',
              });
            } else {
              console.log('[SeasonPass] Sharing is not available on this device. Clone kit saved to:', cloneKitUri);
            }
  ...
  }, [generateBackup, markUserBackupNow]);
  ...
  const restoreFromRecoveryCode = useCallback(async (code: string): Promise<boolean> => {
    ...
    try {
      ...
      const restoredActiveId = data.activeSeasonPassId || (normalized[0]?.id ?? null);
      const restoredMiscEvents: Event[] = Array.isArray((data as any).miscEvents) ? ((data as any).miscEvents as Event[]) : [];
      const restoredLastUserBackupAtISO: string | null = typeof (data as any).lastUserBackupAtISO === 'string' ? String((data as any).lastUserBackupAtISO) : null;
      ...
      // Persist a fresh master backup snapshot to match what we restored.
      await AsyncStorage.setItem(MASTER_BACKUP_KEY, JSON.stringify({
        ...
        miscEvents: restoredMiscEvents,
        lastUserBackupAtISO: restoredLastUserBackupAtISO,
        dataImportedRaw: (data as any).dataImportedRaw ?? null,
        appTheme: (data as any).appTheme ?? (AppColors ?? null),
      } satisfies BackupData));

      console.log('[SeasonPass] Restored from recovery code:', data.seasonPasses.length, 'passes');
      return true;
    } catch (err) {
  ...
  const emailCloneKit = useCallback(async (embedLogos = false): Promise<{ success: boolean; isWeb: boolean }> => {
    try {
      const backup = await generateBackup(embedLogos);
      const code = generateRecoveryCode(backup);
      const backupJsonPretty = JSON.stringify(backup, null, 2);
      ...
      const dateStr = new Date().toISOString().split('T')[0];
      const baseName = `season-pass-clone-kit-${dateStr}`;

      if (Platform.OS === 'web') {
        const fileName = `${baseName}.md`;
        const blob = new Blob([cloneKitMd], { type: 'text/plain' });
        ...
        // Always copy the full kit to clipboard as a web-friendly fallback.
        try {
          await Clipboard.setStringAsync(cloneKitMd);
        } catch {
          // ignore
        }

        const shareText = [
          'Season Pass Clone Kit',
          '',
          'Recovery code (paste into Settings → Restore from Code):',
          code,
          '',
          '(Full Clone Kit Markdown was copied to clipboard.)',
        ].join('\n');

        // Prefer the native share sheet on web (iOS Safari / Android Chrome).
        if (typeof nav.share === 'function') {
          try {
            if (canShareFiles) {
              await nav.share({
                files: [file],
                title: 'Season Pass Clone Kit',
                text: 'Clone kit attached.',
              });
              await markUserBackupNow();
              return { success: true, isWeb: true };
            }

            // If the browser cannot share files, share text instead (Mail/Messages/etc) and rely on clipboard.
            await nav.share({
              title: 'Season Pass Clone Kit',
              text: shareText,
            });
            ...
            const errorName = shareError?.name || 'Unknown';
            // Treat user-cancel as non-fatal.
            if (errorName === 'AbortError') {
              return { success: true, isWeb: true };
            }
            console.log('[EmailCloneKit] Web Share failed:', errorName);
          }
        }

        // Final fallback: download (desktop browsers without share sheets)
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = fileName;
      ...
      // Native: write a .txt file for best email-client compatibility
      const txtFileName = `${baseName}.txt`;
      const txtUri = `${FileSystem.cacheDirectory}${txtFileName}`;
      await FileSystem.writeAsStringAsync(txtUri, cloneKitMd, {
        encoding: FileSystem.EncodingType.UTF8,
      });

      const info = await FileSystem.getInfoAsync(txtUri);
      const size = (info as any).size || 0;
      if (!info.exists || size <= 0) {
        Alert.alert('Clone kit failed', `File not created: ${txtUri}`);
        return { success: false, isWeb: false };
      }
      ...
      console.error('[EmailCloneKit] Error:', error);
      Alert.alert('Error', 'Failed to prepare clone kit.');
      return { success: false, isWeb: Platform.OS === 'web' };
    }
  }, [generateBackup, markUserBackupNow]);

  /**
   * Import schedule from CSV or Excel file
   * This will replace the current schedule for the active season pass
   */
  const importSchedule = useCallback(async (): Promise<{ success: boolean; error?: string; gamesCount?: number }> => {
    try {
      if (!activeSeasonPass) {
        return { success: false, error: 'No active season pass' };
      }

      // Pick document (CSV or Excel)
      ...
      if (!isCSV && !isExcel) {
        return { success: false, error: 'Invalid file type. Please select a CSV or Excel file.' };
      }

      console.log('[ImportSchedule] Reading file:', fileName, 'URI:', fileUri);
      ...
      if (isCSV) {
        ...
        games = await parseScheduleFromCSV(csvContent);
      } else {
        // Read Excel file
        ...
        games = await parseScheduleFromExcel(bytes.buffer);
      }

      // Validate schedule
      const validation = validateSchedule(games);
      if (!validation.valid) {
        console.error('[ImportSchedule] Validation failed:', validation.errors);
        return { success: false, error: `Schedule validation failed:\n${validation.errors.join('\n')}` };
      }

      console.log('[ImportSchedule] Parsed', games.length, 'games successfully');

      // Confirm with user before replacing
      return new Promise((resolve) => {
        Alert.alert(
          'Replace Schedule?',
          `This will replace the current schedule with ${games.length} games from ${fileName}.\n\nYour sales data will be preserved, but unmapped games will lose their sales.\n\nContinue?`,
          [
            {
              text: 'Cancel',
              style: 'cancel',
            ...
            },
            {
              ...
              onPress: async () => {
                try {
                  // Update the season pass with new games
                  const updatedPass = { ...activeSeasonPass, games };
                  const updatedPasses = seasonPasses.map(p =>
                    p.id === activeSeasonPass.id ? updatedPass : p
                  );

                  await saveSeasonPasses(updatedPasses);
                  setSeasonPasses(updatedPasses);

                  console.log('[ImportSchedule] Schedule replaced successfully');
                  Alert.alert('Success', `Schedule replaced with ${games.length} games from ${fileName}`);
                  resolve({ success: true, gamesCount: games.length });
                } catch (error: any) {
                  console.error('[ImportSchedule] Failed to save:', error);
                  resolve({ success: false, error: `Failed to save: ${error.message}` });
                }
              },
            },
          ]
      ...
      console.error('[ImportSchedule] Import failed:', error);
      return { success: false, error: error.message || 'Import failed' };
    }
  }, [activeSeasonPass, seasonPasses, saveSeasonPasses]);
  ...
  const calculateStats = useMemo(() => {
    ...
    Object.entries(activeSeasonPass.salesData).forEach(([gameId, gameSales]) => {
      Object.values(gameSales).forEach(sale => {
        const priceNum = typeof (sale as any).price === 'number' ? (sale as any).price : Number((sale as any).price);
        if (Number.isFinite(priceNum)) totalRevenue += priceNum;
  ...
  removeSaleRecord,
    ...
    // Standalone events (do not affect Analytics)
    miscEvents,
    addMiscEvent,
    updateMiscEvent,
    removeMiscEvent,
